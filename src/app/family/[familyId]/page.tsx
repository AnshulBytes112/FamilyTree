  ...
  return (
    <div className="p-4 md:p-8 max-w-7xl mx-auto space-y-8">
      ...
      <header className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
        <div>
          <h1 className="text-3xl font-heading font-bold text-slate-900 mb-1">{t('header.title', { defaultMessage: 'Dashboard' })}</h1>
          <p className="text-slate-500">{t('header.welcome', { name: user.name })}</p>
        </div>
        <div className="flex flex-wrap gap-2">
          <Link 
            href={`/family/${resolvedParams.familyId}/tree`} 
            className={cn(buttonVariants({ variant: "outline", size: "default" }), "text-slate-700 bg-white")}
          >
          ...
          >
            <Plus size={16} className="mr-2" /> {t('header.addPerson', { defaultMessage: 'Add Person' })}
          </Link>
        </div>
      </header>
        ...
        {stats.map((stat, i) => (
          <Card key={i}>
            <CardContent className="p-5 flex flex-col justify-center">
              ...
              <h3 className="text-4xl font-sans font-medium text-slate-900">{stat.value}</h3>
            </CardContent>
          </Card>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <Card>
          <div className="p-6 pb-4 border-b border-slate-100 flex items-center justify-between">
            <h3 className="font-semibold text-slate-900">{t('recentMembers.title', { defaultMessage: 'Recently Added Members' })}</h3>
          </div>
          <CardContent className="p-0">
            ...
            <div className="p-4 pt-2">
              <Link href={`/family/${resolvedParams.familyId}/people`} className="text-emerald-700 text-sm font-semibold hover:underline">
        ...
        <Card>
          <div className="p-6 pb-4 border-b border-slate-100 flex items-center justify-between">
            <h3 className="font-semibold text-slate-900">{t('recentActivity.title', { defaultMessage: 'Recent Activity' })}</h3>
          </div>
          <CardContent className="p-0">
            <ul className="divide-y divide-slate-50">
            ...
            <div className="p-4 pt-2">
              <button className="text-emerald-700 text-sm font-semibold hover:underline">
